 ...
 */
package com.yitong.android.view.viewflow;
...
//					getPaddingTop() + radius, radius, mPaintInactive);
			canvas.drawRect(leftPadding
					+ (iLoop * circleSeparation) + centeringOffset, getPaddingTop() , 5*radius + leftPadding
					+ (iLoop * circleSeparation) + centeringOffset , getPaddingTop() + 2*radius, mPaintInactive);
		}
		float cx = 0;
		if (flowWidth != 0) {
...
//				+ radius, radius + activeRadius, mPaintActive);
		canvas.drawRect(leftPadding + cx + centeringOffset, getPaddingTop(), 5*radius + leftPadding + cx+centeringOffset , getPaddingTop()
				+ 2*radius, mPaintActive);
	}
	...
	private int measureWidth(int measureSpec) {
		...
		// Calculate the width according the views count
			...
			float temp = circleSeparation - 5*radius;
			result = (int) (getPaddingLeft() + getPaddingRight()
					+ (count * 5 * radius) + (count - 1) * temp + 1);
